...
export default function CounselPage() {
  ...
  const loadSessions = async () => {
    const res = await fetch('http://localhost:8000/api/v1/counsel/sessions', { headers: headers() });
    if (res.ok) setSessions(await res.json());
  };

  const loadMessages = async (sessionId: string) => {
    const res = await fetch(`http://localhost:8000/api/v1/counsel/sessions/${sessionId}/messages`, { headers: headers() });
  ...
  const startSession = async () => {
    ...
    const res = await fetch('http://localhost:8000/api/v1/counsel/sessions', {
  ...
  const sendMessage = async () => {
    ...
    setInputText('');
    setLoading(true);
    const res = await fetch(`http://localhost:8000/api/v1/counsel/sessions/${activeSession.id}/messages`, {
      method: 'POST', headers: headers(),
  ...
  const closeSession = async () => {
    ...
    await fetch(`http://localhost:8000/api/v1/counsel/sessions/${activeSession.id}/close`, {
  ...
  if (view === 'home') return (
    <div>
      ...
      <div style={{
        ...
        padding: '40px 28px 36px', color: 'white'
      }}>
        <div style={{ maxWidth: 860, margin: '0 auto' }}>
          <div style={{
            display: 'inline-block', background: 'rgba(255,255,255,.2)',
            borderRadius: 20, padding: '4px 14px', fontSize: 13, marginBottom: 16
          }}>전문 상담사와 1:1 익명 상담</div>
        ...
        <div style={{ marginBottom: 36 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
            {[
              ...
              { n: 2, title: '상담사 배정',   desc: '전문 상담사가 즉시 배정되어 대화를 시작합니다.' },
              ...
              <div key={s.n} style={{
                ...
                <div style={{
                  width: 36, height: 36, background: '#4F8EF7', color: 'white',
                  borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontWeight: 700, fontSize: 15, margin: '0 auto 12px'
                }}>{s.n}</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }}>
            {CONCERN_CATEGORIES.map(cat => (
              <div key={cat.id}
                ...
                style={{
                  background: '#F8F9FA', borderRadius: 14, padding: '24px 16px',
                  textAlign: 'center', cursor: 'pointer',
                  border: '2px solid transparent', transition: 'all .2s',
                  boxShadow: '0 1px 4px rgba(0,0,0,.04)'
                }}
                onMouseOver={e => { (e.currentTarget as HTMLElement).style.borderColor = cat.color; (e.currentTarget as HTMLElement).style.transform = 'translateY(-4px)'; (e.currentTarget as HTMLElement).style.boxShadow = `0 8px 24px ${cat.color}22`; (e.currentTarget as HTMLElement).style.background = 'white'; }}
                onMouseOut={e => { (e.currentTarget as HTMLElement).style.borderColor = 'transparent'; (e.currentTarget as HTMLElement).style.transform = ''; (e.currentTarget as HTMLElement).style.boxShadow = '0 1px 4px rgba(0,0,0,.04)'; (e.currentTarget as HTMLElement).style.background = '#F8F9FA'; }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {sessions.map(s => (
                <div key={s.id}
                  ...
                  onMouseOver={e => { if (s.status !== 'closed') (e.currentTarget as HTMLElement).style.boxShadow = '0 4px 16px rgba(79,142,247,.1)'; }}
                  onMouseOut={e => { (e.currentTarget as HTMLElement).style.boxShadow = '0 1px 4px rgba(0,0,0,.04)'; }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <textarea
                style={{ width: '100%', border: '1px solid #dee2e6', borderRadius: 8, padding: '12px', fontFamily: 'inherit', fontSize: '.88rem', resize: 'vertical', minHeight: 100, outline: 'none' }}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 64px)' }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontWeight: 700, fontSize: '.95rem' }}>{activeSession?.counselor_name}</div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px', background: '#F8F9FA', display: 'flex', flexDirection: 'column', gap: 14 }}>
        {/* 상담 주제 표시 */}
        <div style={{ textAlign: 'center', padding: '12px 20px', background: '#e8f4fd', borderRadius: 12, fontSize: '.82rem', color: '#4F8EF7', margin: '0 auto', maxWidth: 400 }}>
      ...
      <div style={{
        ...
        padding: '14px 20px', display: 'flex', gap: 10, flexShrink: 0
      }}>
        <input
